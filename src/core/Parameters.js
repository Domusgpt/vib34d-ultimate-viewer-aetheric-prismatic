...
} from '../variations/variationPresets.js';
...
export class ParameterManager {
    constructor() {
        ...
        this.geometryNames = GeometryLibrary.getGeometryNames().map(name => `${name} LATTICE`);

        this.presetVariationCount = DEFAULT_VARIATION_COUNT;
    ...
    updateVariationInfo() {
        ...
        if (variationDisplay) {
            const definition = getDefaultVariationDefinition(this.params.variation);
            if (definition) {
                const geometryName = `${GeometryLibrary.getGeometryName(definition.geometry)} LATTICE`;
                variationDisplay.textContent = `${this.params.variation + 1} - ${geometryName} ${definition.level + 1}`;
            } else {
                variationDisplay.textContent = `${this.params.variation + 1} - CUSTOM VARIATION`;
            }
        }
    }
    ...
    generateVariationParameters(variationIndex) {
        ...
        if (variationIndex < presetCount) {
            const definition = getDefaultVariationDefinition(variationIndex);
            if (definition) {
