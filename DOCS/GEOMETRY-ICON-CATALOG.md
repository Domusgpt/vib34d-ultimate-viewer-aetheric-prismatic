...
Validate the outputs with `npm run geometry:verify` to confirm the manifest entries, filenames, and preset sections remain aligned.

Generate the complementary written compendium with `npm run geometry:report` to produce `DOCS/GEOMETRY-PRESET-COMPENDIUM.md`, which cross-links the icons with preset labels, slot indices, and parameter envelopes for quick stakeholder reviews.

Render the interactive offline gallery with `npm run geometry:gallery`; the command publishes `docs/geometry-gallery/index.html` and a JSON manifest so reviewers can browse preset coverage without launching the viewer or installing Playwright.
