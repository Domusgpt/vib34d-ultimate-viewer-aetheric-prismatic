...
- Generate a Markdown snapshot of the current coverage matrix with `npm run geometry:report`; the script writes `DOCS/GEOMETRY-ASSET-STATUS.md` so reviewers can diff geometry, icon, and preset counts without re-running the verification tooling locally.
