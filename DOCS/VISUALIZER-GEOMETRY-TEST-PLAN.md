...
- Run `npx playwright test tests/visual-geometry-presets.spec.js --project=chromium` to capture the geometry grids and assert dataset metadata for the faceted, quantum, and holographic systems.
...
- Review the Playwright HTML report (`npx playwright show-report`) for hover details, console logs, and metadata snapshots emitted by the automation run.
