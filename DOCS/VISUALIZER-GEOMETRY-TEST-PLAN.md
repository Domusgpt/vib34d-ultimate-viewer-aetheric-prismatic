...
- Build the interactive static gallery via `npm run geometry:gallery` to review preset labels, icons, and parameter envelopes inside `docs/geometry-gallery/` when browser automation cannot be executed.
...
- `npm run geometry:report` rebuilds the generated dossier in `DOCS/GEOMETRY-PRESET-COMPENDIUM.md`, summarising preset labels, index ranges, and parameter envelopes for stakeholders reviewing geometry coverage offline.
